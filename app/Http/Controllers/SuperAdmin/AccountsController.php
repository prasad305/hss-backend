    ...
    public function accountFilter(Request $request)
    {
        ...
        if ($module == "1") {
            ...
            foreach ($live_chat as $leSess) {
                $userReg[$i] = LiveChatRegistration::where('live_chat_id', $leSess['id'])->count();
                $i++;
            }
            ...
            foreach ($greetings as $leSess) {
                $userReg[$i] = GreetingsRegistration::where('greeting_id', $leSess['id'])->count();
                $i++;
            }
            ...
            foreach ($meetup_event as $leSess) {
                $userReg[$i] = MeetupEventRegistration::where('meetup_event_id', $leSess['id'])->count();
                $i++;
            }
        ...
        // }
            ...
            foreach ($qna as $leSess) {
                $userReg[$i] = QnaRegistration::where('qna_id', $leSess['id'])->count();
                $i++;
            }
            ...
            foreach ($marketPlace as $leSess) {
                $userReg[$i] = MarketplaceOrder::where('marketplace_id', $leSess['id'])->count();
                $i++;
            }
            ...
            foreach ($souvenir as $leSess) {
                $userReg[$i] = SouvenirApply::where('souvenir_id', $leSess['id'])->count();
                $i++;
            }
