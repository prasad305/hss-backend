...
class JuryAuditionController extends Controller
{
    ...
    public function singleAuditionVideoAssessmentWithRound($audition_id, $audition_round_info_id)
    {
        $audition = Audition::find($audition_id);
        $auditionRoundInfo  = AuditionRoundInfo::find($audition_round_info_id);

        if (AuditionUploadVideo::where([['round_info_id', $audition_round_info_id], ['audition_id', $audition_id], ['group_b_jury_id', auth()->user()->id]])->count() > 0) {
            $myGroup = 'b';
            $auditionUploadVideos =  AuditionUploadVideo::where([['round_info_id', $audition_round_info_id], ['audition_id', $audition_id], ['group_b_jury_id', auth()->user()->id],['group_b_jury_mark', null]])->orderBy('updated_at','DESC')->get();
            $markedVideos =  AuditionUploadVideo::where([['round_info_id', $audition_round_info_id], ['audition_id', $audition_id], ['group_b_jury_id', auth()->user()->id],['group_b_jury_mark', '!=',null]])->orderBy('updated_at','DESC')->get();
        } else {
            $myGroup = 'c';
            $auditionUploadVideos =  AuditionUploadVideo::where([['round_info_id', $audition_round_info_id], ['audition_id', $audition_id], ['group_c_jury_id', auth()->user()->id],['group_c_jury_mark', null]])->orderBy('updated_at','DESC')->get();
            $markedVideos =  AuditionUploadVideo::where([['round_info_id', $audition_round_info_id], ['audition_id', $audition_id], ['group_c_jury_id', auth()->user()->id],['group_c_jury_mark', '!=',null]])->orderBy('updated_at','DESC')->get();
        }
        return response()->json([
            'status' => 200,
            'audition' => $audition,
