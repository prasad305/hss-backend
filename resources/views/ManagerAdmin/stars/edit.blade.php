            ...
            <select name="sub_category_id" id="sub_category_id" class="form-control">
                ...
                @if (isset($sub_categories[0]))
                    @foreach ($sub_categories as $key => $subCategory)
                        <option {{ $subCategory->id == $star->sub_category_id ? 'selected' : '' }}
    ...
    <div class="form-group row">
        <div class="col-md-6">
            ...
            <input type="text" class="form-control" id="profit" name="profit"
                placeholder="Enter  Profit Share in percentage" value="{{ $star->profitShare->profit }}">
        ...
        $.ajax({
            ...
            error: function(data) {
                $.each(data.responseJSON.errors, function(key, value) {
                    ErrorMessage(key,
                    value); //this function on master page for showing error message
                });
            }
        });
