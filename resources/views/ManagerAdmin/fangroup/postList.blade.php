            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <a class="btn btn-success btn-sm" style="float: right;"
                        href="{{ route('managerAdmin.dashboard.fanGroup') }}"><i class=" fa fa-arrow"></i>&nbsp;Go
                        Back</a>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                            ...
                            @foreach ($postList as $key => $post)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{ $post->my_admin->first_name }}
                                        {{ $post->my_admin->last_name }} VS
                                        {{ $post->another_admin->first_name }}
                                        {{ $post->another_admin->last_name }}
                                    </td>
                                    <td>

                                        {{ $post->my_superstar->first_name }}
                                    ...
                                    <td style="width: 150px">
                                        <a href="{{ route('managerAdmin.fangroup.details', [$post->id]) }}"
                                            class="btn btn-sm btn-success"> <i class="fa fa-eye"></i></a>
                                        <a class="btn btn-sm btn-info"
                                            onclick="Show('Edit Post','{{ route('managerAdmin.fangroup.edit', $post->id) }}')"><i
                                                class="fa fa-edit text-white"></i></a>
                                        {{-- <button class="btn btn-sm btn-danger" onclick="delete_function(this)"
                                            value="{{ route('managerAdmin.fanGroup.destroy', $post->id) }}"><i
