<form  id="edit-form" enctype="multipart/form-data">
        ...
        <select name="sub_category_id" id="sub_category_id" class="form-control">
            ...
            @if(isset($sub_categories[0]))
                @foreach ($sub_categories as $key => $subCategory)
                    <option {{$subCategory->id == $jury->sub_category_id ? 'selected' : ''}} value="{{$subCategory->id}}">{{$subCategory->name}}</option>
                @endforeach
            @endif
        </select>
    ...
    <div class="row form-group">
        <div class="col-md-12">
            ...
            <textarea name="terms_and_condition" class="form-control" id="summernote">{!! $jury->jury ? $jury->jury->terms_and_condition : '' !!}</textarea>
         </div>
    </div>
    ...
    <button type="submit" class="btn btn-success" id="btnUpdateData"><i class="fa fa-save"></i>&nbsp; Update Jury</button>
