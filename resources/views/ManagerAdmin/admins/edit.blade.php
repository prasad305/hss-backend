<form  id="edit-form" enctype="multipart/form-data">
        ...
        <select name="sub_category_id" id="sub_category_id" class="form-control">
            ...
            @if(isset($sub_categories[0]))
                @foreach ($sub_categories as $key => $subCategory)
                    <option {{$subCategory->id == $admin->sub_category_id ? 'selected' : ''}} value="{{$subCategory->id}}">{{$subCategory->name}}</option>
                @endforeach
            @endif
        </select>
    ...
    <button type="submit" class="btn btn-success" id="btnUpdateData"><i class="fa fa-save"></i>&nbsp; Update Admin</button>
